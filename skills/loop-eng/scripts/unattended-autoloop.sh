...
# let a session commit. Refuse up front instead. 78 = EX_CONFIG, the same code
# these scripts use for every other unrunnable-configuration exit.
...
# non-bash shell (`sh unattended-autoloop.sh`) dies on the ARRAY SUBSCRIPT in
# the test below with a bare "Bad substitution" — the `:-0` default never gets a
